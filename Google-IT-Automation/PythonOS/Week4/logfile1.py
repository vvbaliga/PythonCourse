# We're using the same syslog, and we want to display the date, time, and 
# process id that's inside the square brackets. We can read each line of 
# the syslog and pass the contents to the show_time_of_pid function. 
...
def show_time_of_pid(line):
  pattern = r"(Jul)|\d \d:\d:\d | CRON\((\w+)\)$"
  result = re.search(pattern, line)
  return (result[0] + result[1] + 'pid: ' + result[2])
